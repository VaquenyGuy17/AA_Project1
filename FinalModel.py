...
def val_poly_regression(X_train, y_train, X_val, y_val, regressor=RidgeCV(), degrees=range(1, 8), max_features=None):
    best_rmse = float('inf')
    best_model = None
    best_degree = None
...
# Function to create new ratio, inverse and distance features
def create_new_features(X):
    X['ratio_x1_y1'] = X['x_1'] / (X['y_1'] + 1e-5)
    X['ratio_x2_y2'] = X['x_2'] / (X['y_2'] + 1e-5)

    X['inv_x1'] = 1 / (X['x_1'] + 1e-5)
    X['inv_y1'] = 1 / (X['y_1'] + 1e-5)

    X['dist_1_2'] = np.sqrt((X['x_1'] - X['x_2']) ** 2 + (X['y_1'] - X['y_2']) ** 2)
    X['dist_2_3'] = np.sqrt((X['x_2'] - X['x_3']) ** 2 + (X['y_2'] - X['y_3']) ** 2)
    X['dist_1_3'] = np.sqrt((X['x_1'] - X['x_3']) ** 2 + (X['y_1'] - X['y_3']) ** 2)
...
# Updated polynomial regression function with new features and knn regressor
def polynomialRegression_with_new_features(X_train, y_train, X_test, y_test):
    ...
    print(f"Best RMSE with new features: {best_rmse}")
    ...
    print(f"Test RMSE with new features: {root_mean_squared_error(y_sample_test, result)}")
    ...
    print(f"Best feature set with new features: {best_features}")
    ...
    print(f"Best RMSE with reduced features: {best_rmse_final}")

    best_model_ultimate, best_rmse_ultimate, best_k_ultimate = validate_knn_regression(X_train_reduced, y_train_reduced, X_val_reduced, y_val_reduced)
    pipeline = best_model_ultimate
    pipeline.fit(X_train_reduced, y_train_reduced)
    result = pipeline.predict(X_val_reduced)
    print(f"Test RMSE with reduced features: {root_mean_squared_error(y_val_reduced, result)}")
    plot_y_yhat(y_val_reduced.to_numpy(), result)
    ...
    for (feature1, feature2) in to_drop:
        if feature1 not in dropped_features and feature2 not in dropped_features:
            for feature_to_drop in [feature1, feature2]:
                X_train_reduced_tmp = X_train_reduced.drop(columns=[feature_to_drop])
                X_val_reduced_tmp = X_val_reduced.drop(columns=[feature_to_drop])

                pipeline = best_model
                pipeline.fit(X_train_reduced_tmp, y_train)
                result = pipeline.predict(X_val_reduced_tmp)
                rmse = root_mean_squared_error(y_val, result)

                if rmse < best_rmse:
                    best_rmse = rmse
                    best_features = X_train_reduced_tmp.columns
                    dropped_features.add(feature_to_drop)
                    X_train_reduced = X_train_reduced_tmp
                    X_val_reduced = X_val_reduced_tmp
                    print(f"Eliminated: {feature_to_drop}, RMSE: {rmse}")

    return best_features, best_rmse, X_train_reduced, X_val_reduced, y_train, y_val
...
polynomialRegression_with_new_features(X_train, y_train, X_test, y_test)
